...
var GumgaChips = {
  ...
  controller: ['$scope', '$attrs', '$timeout', '$element', function ($scope, $attrs, $timeout, $element) {
    ...
    var getOthersChips = function getOthersChips() {
      var dates = angular.element('gumga-chips');
      var others = dates.filter(function (i) {
        return !angular.equals(dates[i], $element[0]);
      });
      return others;
    };

    var closeOthersChips = function closeOthersChips() {
    ...
    ctrl.showAddNewItem = function () {
      return ctrl.tagging && ctrl.inputValue != undefined && ctrl.inputValue != '' && ctrl.inputValue != null && ctrl.inputValue.trim() != '' && $element.find('ul[ng-transclude="options"]').hasClass('open');
    };

    ctrl.showNoOptions = function () {
      return $element.find('gumga-chips-option').find('li.option-container').length == 0 && $element.find('ul[ng-transclude="options"]').hasClass('open');
    };
    ...
    ctrl.removeFocusedItems = function () {
      ...
      angular.forEach($element.find('gumga-chips-item div.item-container'), function (itemContainer) {
    ...
    ctrl.removeFocusedOptions = function () {
      angular.forEach($element.find('gumga-chips-option').find('li.option-container'), function (optionContainer) {
        angular.element(optionContainer).removeClass('option-focused');
      });
    };

    ctrl.removeFocusInput = function () {
      $element.find('input').blur();
    };

    ctrl.addFocusInput = function (ignoreClick) {
      $timeout(function () {
    ...
    ctrl.applyFocused = function (item) {
    ...
    ctrl.getLastItemEnabled = function (item) {
    ...
    ctrl.analyseIfContainsOtherItem = function () {
      $timeout(function () {
        var items = ctrl.getLastItemEnabled($element.find('gumga-chips-item').last());
        if (items && items[0]) {
    ...
    ctrl.applyFocusedOrRemoveItem = function (items, evt) {
      var item = angular.element(items[0]).find('.item-container');
    ...
    ctrl.handlingBackspace = function (evt) {
      var itemFocused = $element.find('gumga-chips-item div.item-container.item-focused');
    ...
    ctrl.getFirstOption = function () {
      return $element.find('gumga-chips-option').find('li.option-container').first();
    };

    ctrl.getLastOption = function () {
      return $element.find('gumga-chips-option').find('li.option-container').last();
    };

    var setScrollInOptionFocused = function setScrollInOptionFocused() {
      var liFocused = $element.find('gumga-chips-option').find('li.option-container.option-focused');
      if (liFocused && liFocused[0]) {
        var container = $element.find('ul.options');
        var scrollTo = liFocused;
        container.animate({
    ...
    ctrl.nextOption = function (evt, elm) {
      evt.preventDefault();
      evt.stopPropagation();
      $timeout(function () {
        var next = elm.next();
    ...
    ctrl.prevOption = function (evt, elm) {
      evt.preventDefault();
      evt.stopPropagation();
      var previous = elm.prev();
    ...
    ctrl.handligButtonUp = function (evt) {
      var liFocused = $element.find('gumga-chips-option').find('li.option-container.option-focused');
      if (liFocused && liFocused[0]) {
    ...
    ctrl.handligButtonDown = function (evt) {
      var liFocused = $element.find('gumga-chips-option').find('li.option-container.option-focused');
      if (liFocused && liFocused[0]) {
        ctrl.nextOption(evt, angular.element(liFocused[0].parentNode));
      } else {
        var options = $element.find('gumga-chips-option').find('li.option-container.option-enabled');
        if (options.length > 0) {
    ...
    ctrl.hasOptionAvaliable = function () {
      $scope.asda = true;
      return $element.find('gumga-chips-option').find('li.option-container').length > 0 && $element.find('gumga-chips-option').find('li.option-container:not(.ng-hide)').length > 0;
    };

    var applyFocusedFirstOption = function applyFocusedFirstOption() {
      $element.find('li.option-container').removeClass('option-focused');
      if (ctrl.hasOptionAvaliable()) {
        var ell = getFirstOption(Array.from($element.find('gumga-chips-option')));
        ell.find('li.option-container').addClass('option-focused');
      }
    };

    var getFirstOption = function getFirstOption(elms) {
      var el = angular.element(elms[0]);
    ...
    $scope.$watch('$ctrl.ngDisabled', function (disabled) {
      if (!disabled) return;
      ctrl.removeFocusedItems();
      ctrl.removeFocusedOptions();
    });

    ctrl.exceededMaxItems = function () {
...
var Item = {
  ...
  controller: ['$scope', '$attrs', '$timeout', '$element', function ($scope, $attrs, $timeout, $element) {
    ...
    ctrl.applyFocused = function (evt) {
      if (ctrl.gumgaChipsCtrl.ngDisabled || ctrl.ngDisabled) return;
      ctrl.gumgaChipsCtrl.applyFocused($element.find('div.item-container'));
    };

    ctrl.moveFocusToLeft = function (evt) {
      var previousElement = angular.element(evt.target.parentNode).prev();
    ...
    ctrl.moveFocusToRight = function (evt) {
      var nextElement = angular.element(evt.target.parentNode).next();
    ...
    var getPrev = function getPrev(elm) {
    ...
    var getNext = function getNext(elm) {
      if (elm.find('div.item-container')[0] && elm.find('div.item-container')[0].classList.contains('item-disabled')) {
        if (elm.next) {
...
var _option = __webpack_require__(2);
...
var _module = angular.module('gumga.chips', []).component('gumgaChips', _chips2.default).component('gumgaChipsOption', _option2.default).component('gumgaChipsItem', _item2.default);
